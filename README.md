...
Erlaubte Werte für Schalter sind `yes` und `no`. `COLOR_MODE` akzeptiert `always` und `never`; `UPDATE_MODE` akzeptiert `notify` und `automatic`.
...
Foxly MOTD is a fast, updateable system dashboard for Debian and Ubuntu SSH logins. Package metadata is refreshed by a systemd timer, so the login path performs no APT or network requests.
...
Configuration is stored in `/etc/default/foxly-motd`. Automatic software updates are disabled by default; the daily timer only records and displays available releases.
